  ...
  methods: {
    getwaktu: function getwaktu() {
      ...
      this.$http.post('/ujian/index', {
        ...
        if (getData.ujian.ujian_siswa) {
          var today = new Date();
          var d = _this.addMinutes(today, getData.ujian.ujian_siswa.waktu);
          _this.yourEndDate = _this.setDate(d);
        }
        if (replace) {
    ...
    getSoal: function getSoal(ujian_id, nomor, replace) {
      ...
      this.$http.post('/ujian/index', {
        ...
        var getData = response.data;
        _this2.isBusy = false;
        _this2.data = getData.soal;
        _this2.nomor = nomor;
        _this2.ujian_id = ujian_id;
        _this2.jumlah_soal = getData.ujian.soal_count;
        if (getData.soal.jawaban_siswa) {
          console.log(getData.soal.jawaban_siswa.is_ragu);
          _this2.selected = getData.soal.jawaban_siswa.opsi;
          _this2.ragu = getData.soal.jawaban_siswa.is_ragu ? true : false;
          _this2.button_ragu = getData.soal.jawaban_siswa.is_ragu ? 'warning' : 'secondary';
        }
        if (getData.ujian.ujian_siswa) {
          var today = new Date();
          var d = _this2.addMinutes(today, getData.ujian.ujian_siswa.waktu);
        ...
        if (replace) {
          _this2.$router.replace({
            name: "ujian",
            params: {
              ujian_id: _this2.$route.params.ujian_id
            },
            query: {
              nomor: nomor
            }
    ...
    lanjut: function lanjut(soal_id, nomor, nav) {
      ...
      this.$http.post('/ujian/simpan', {
        ...
        _this3.getSoal(_this3.$route.params.ujian_id, nomor_ujian, true);
      });
    },
