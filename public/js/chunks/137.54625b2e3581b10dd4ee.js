  ...
  methods: {
    ...
    batalkan: function batalkan(item) {
      ...
      this.$swal({
        ...
        if (result.value) {
          _this3.$http.post('/referensi/batalkan-pd', {
            status: 'lulus',
    ...
    handleSubmit: function handleSubmit() {
      ...
      this.$http.post('/referensi/nomor-ijazah', {
        ...
        if (data.errors) {
          if (data.errors.urut) {
            _this4.nomor_seriState = false;
            _this4.feedback = data.errors.urut[0];
          }
          if (data.errors.tanggal) {
            _this4.tanggalState = false;
            _this4.feedback_tanggal = data.errors.tanggal[0];
          }
        } else {
          _this4.$swal({
            icon: data.icon,
