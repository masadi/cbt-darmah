  ...
  methods: {
    ...
    batalkan: function batalkan(item) {
      ...
      this.$swal({
        ...
        if (result.value) {
          _this3.$http.post('/blangko/batalkan-verifikasi', {
            blangko_id: _this3.blangko_id
          }).then(function (response) {
            var data = response.data;
            _this3.$swal({
              icon: data.icon,
              ...
              _this3.$refs['modal-preview'].hide();
    ...
    preview: function preview(item) {
      ...
      this.$http.get('/blangko/preview', {
        ...
        console.log(getData);
        _this4.$refs['modal-preview'].show();
        return false;
        if (getData) {
          _this4.previewUrl = 'https://drive.google.com/file/d/' + getData.file_id + '/preview';
          _this4.$refs['modal-preview'].show();
        } else {
          _this.$swal({
            icon: 'error',
