...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    generateUser: function generateUser(item) {
      ...
      this.$swal({
        ...
        if (result.value) {
          _this3.$http.post('/referensi/generate-pengguna', {
            sekolah_id: item.sekolah_id
          }).then(function (response) {
    ...
    makeFire: function makeFire(item) {
      ...
      this.$swal({
        ...
        if (result.value) {
          _this4.$http.post('/referensi/make-fire', {
            fire: item.user.fire,
  ...
  pollData: function pollData() {
    ...
    this.polling = setInterval(function () {
      _this6.$http.get('/referensi/hitung-pd', {
        params: {
          sekolah_id: _this6.user.sekolah_id
        }
      }).then(function (response) {
        _this6.value = response.data.persentase;
      });
    }, 1000);
  }
});
