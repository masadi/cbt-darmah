  ...
  <div>
    ...
    <b-table responsive striped :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" show-empty :busy="isBusy">
      ...
      <template v-slot:cell(kategori)="row">
        {{row.item.kategori.title}}
      </template>
      <template v-slot:cell(status)="row">
        <b-badge variant="success" v-if="row.item.status">Terjawab</b-badge>
        <b-badge variant="danger" v-else>Antrean</b-badge>
      </template>
      <template v-slot:cell(actions)="row">
        <b-button variant="primary" size="sm" :to="{name: 'detil-pengaduan', params: {pengaduan_id: row.item.id}}"><font-awesome-icon icon="fa-solid fa-magnifying-glass" /> Detil</b-button>
      </template>
    </b-table>
    <b-row class="mt-2">
      <b-col md="6">
