<template>
  <b-card no-body>
    ...
    <b-card-body v-if="!hasRole('diskab')">
      <p>Pada hari {{hari}} tanggal {{tanggal}} bulan {{bulan}} tahun {{tahun}}, telah dilaksanakan kegiatan pemusnahan Blangko Ijazah oleh:</p>
      <table>
        <tr>
          <td>Nama</td>
      ...
      <p class="mt-1 mb-0" v-show="fire">Dengan rincian:</p>
      ...
      <p class="mt-1 mb-0">Yang disaksikan oleh:</p>
      <table>
        <tr>
          <td>Nama</td>
    ...
    <b-modal id="berita-acara" ref="modal" title="Edit Data Berita Acara Pemusnahan Blangko Ijazah" @show="resetModal" @hidden="resetModal" @ok="handleOk" ok-title="Simpan" cancel-title="Batal">
      <form ref="form" @submit.stop.prevent="handleSubmit">
        ...
        <b-form-group label="Instansi" label-for="instansi_2" :invalid-feedback="feedback_instansi_2" :state="instansi_2State">
          ...
          <b-form-select v-model="instansi_2" :options="options"></b-form-select>
        </b-form-group>
    ...
    <b-modal id="upload-berita-acara" title="Unggah Berita Acara Pemusnahan Blangko Ijazah">
    ...
    getBeritaAcara(){
      this.$http.get('/referensi/berita-acara', {
        ...
        if(this.data){
          ...
          this.link_cetak = app_url+'/cetak/berita-acara/pemusnahan/'+this.user.user_id
        } else {
