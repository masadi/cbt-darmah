<template>
  <b-card no-body>
    <b-card-header>
      <b-card-title>
        <h2>Berita Acara Penerimaan Blangko Ijazah</h2>
      </b-card-title>
    ...
    <b-card-body v-if="!hasRole('diskab')">
      ...
      <table class="table table-bordered">
        <tr>
          <td rowspan="3" width="30%">Blangko Ijazah SMK Program 3 Tahun</td>
        </tr>
        <tr>
          <td v-if="jumlah_3">{{jumlah_3}} lembar Blangko Ijazah</td>
          <td v-else>-</td>
        </tr>
        <tr>
          <td v-if="jumlah_3">M-SMK/K13-3/23/{{start_3.padStart(7, '0')}} <br>s/d<br> M-SMK/K13-3/23/{{end_3.padStart(7, '0')}}</td>
          <td v-else>-</td>
        </tr>
        <tr>
        ...
        <tr>
          <td v-if="jumlah_4">{{jumlah_4}} lembar Blangko Ijazah</td>
          <td v-else>-</td>
        </tr>
        <tr>
          <td v-if="jumlah_4">M-SMK/K13-4/23/{{start_4.padStart(7, '0')}} <br>s/d<br> M-SMK/K13-4/23/{{end_4.padStart(7, '0')}}</td>
          <td v-else>-</td>
        </tr>
      </table>
    ...
    <b-modal id="berita-acara" ref="modal" title="Edit Data Berita Acara Penerimaan Blangko Ijazah" @show="resetModal" @hidden="resetModal" @ok="handleOk" ok-title="Simpan" cancel-title="Batal">
      <form ref="form" @submit.stop.prevent="handleSubmit">
        ...
        <b-form-group label="Kabupaten/Kota" label-for="kabupaten" :invalid-feedback="feedback_kabupaten" :state="kabupatenState" v-show="!hasRole('disprov')">
    ...
    <b-modal id="upload-berita-acara" title="Unggah Berita Acara Penerimaan Blangko Ijazah">
    ...
    getBeritaAcara(){
      this.$http.get('/referensi/berita-acara', {
        ...
        if(this.data){
          ...
          this.link_cetak = app_url+'/cetak/berita-acara/penerimaan/'+this.user.user_id
        } else {
          ...
          if(this.hasRole('disprov')){
            this.jabatan = 'Kepala Dinas Provinsi'
            this.instansi = 'Dinas Pendidikan '+this.user.name
        ...
        if(blangko_3_tahun){
          ...
          this.start_3 = (blangko_3_tahun.start) ? blangko_3_tahun.start.toString() : 0
          this.end_3 = (blangko_3_tahun.end) ? blangko_3_tahun.end.toString() : 0
        }
        let blangko_4_tahun = response.data.blangko_4_tahun
        if(blangko_4_tahun){
          this.jumlah_4 = blangko_4_tahun.jumlah
          this.start_4 = (blangko_4_tahun.start) ? blangko_4_tahun.start.toString() : 0
          this.end_4 = (blangko_4_tahun.end) ? blangko_4_tahun.end.toString() : 0
        }
