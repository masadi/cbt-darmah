      ...
      <b-form ref="form_profile" @submit.stop.prevent="handleSubmit">
        ...
        <b-card-body v-else>
          ...
          <b-row class="mb-1">
            ...
            <b-col sm="9">
              <b-form-input id="no_hp" type="text" v-model="no_hp" :invalid-feedback="feedback_no_hp" :state="no_hp_State"></b-form-input>
              <p v-show="feedback_no_hp" class="text-danger">{{feedback_no_hp}}</p>
            </b-col>
          </b-row>
...
export default {
  ...
  created() {
    this.$http.get('/referensi/users/profile').then(response => {
      this.isBusy = false
    ...
    handleSubmit(){
      ...
      this.$http.post('/referensi/users/update-profile', {
        ...
        if(data.errors){
          ...
          this.no_hp_State = (data.errors.no_hp) ? false : null
          this.feedback_no_hp = (data.errors.no_hp) ? data.errors.no_hp[0] : ''
        } else {
