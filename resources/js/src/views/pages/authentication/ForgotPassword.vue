<template>
  <div class="auth-wrapper auth-v2">
    <b-row class="auth-inner m-0">
      ...
      <!-- Left Text-->
      <b-col lg="8" class="d-none d-lg-flex align-items-center p-5" style="background: #E0E0E2;">
            ...
            <b-form class="auth-forgot-password-form mt-2" @submit.prevent="validationForm">
              <b-form-group label="Email" label-for="forgot-password-email">
                <validation-provider #default="{ errors }" name="Email" rules="required|email">
                  <b-form-input id="forgot-password-email" v-model="userEmail" :state="errors.length > 0 ? false:null" name="forgot-password-email" placeholder="email@ditpsmk.net"/>
                  <small class="text-danger">{{ errors[0] }}</small>
                </validation-provider>
              </b-form-group>
              <b-button type="submit" variant="primary" block v-show="show">
                Kirim link reset
              </b-button>
      ...
      this.$refs.simpleRules.validate().then(success => {
        ...
        if (success) {
          this.$toast({
            ...
            props: {
              title: 'Link reset password berhasil dikirim. Silahkan cek inbox/spam email Anda!',
              icon: 'EditIcon',
              variant: 'success',
            },
          })
